     ...
     **************************************************************************/
     ...
     function prepare_items() {
        ...
        foreach ($all_posts as $value) {
            ...
            preg_match('~<a .*?href=[\'"]+(.*?)[\'"]+.*?>(.*?)</a>~ims', $content, $result);
            
            preg_match_all('~<a(.*?)href="([^"]+)"(.*?)>~', $content, $matches);            // var_dump(preg_match('~^https?://[a-z0-9-]+(?:\.[a-z0-9-]+)*(?::[0-9]+)?(?:/[^\W\d]+(?:/\d+)?)?$~gm', $url));
            ...
            foreach ($matches[2] as $url){
                // $pattern = "~^https?://[a-z0-9-]+(?:\.[a-z0-9-]+)*(?::[0-9]+)?(?:/[^\W\d]+(?:/\d+)?)?$~";
                ...
                if (preg_match("/^$regex$/i", $url)) {
                    switch ($url){
                        ...
                        case strpos($url, "https"):
                            $estado = 'Enlace seguro';
            ...
            array_push($post_data, $data_post);
            // var_dump($data_post);
            // var_dump($value);
            };
